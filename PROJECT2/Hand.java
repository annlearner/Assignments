...
//		System.out.println(numberOfClubs);
	...
	public Card playAcards(Game game,Trick trick){
    	...
    	if(trick.getCurrentSize() == 0){
    	  ...
    	  //shortest != 0 a.only heart? b.
    		...
    		else{//System.out.println("1111.4444" );
			index =findLowest(game);}
	...
	//find the lowest in all cards, see whether heart is broken		
	public int findLowest(Game game){
	   Card lowest = new Card(Integer.MAX_VALUE,-1);
	   int index = -1;
   	   int isHeart = 0;
	// if( "hearts have been broken")怎么实现
   	   if(game.getHearts()){
   	      for(int i = 0; i < getCurrentSize(); i++){
			   if(getCard(i).getSuit() != 2 ){
				   if(lowest.getNum() > getCard(i).getNum()){
				           lowest = getCard(i);
				            index = i;
				         }
			  }else{
	//to test weather there is only heart in the hand
				           isHeart++;	
				         }
   	          }
   	   }else{
